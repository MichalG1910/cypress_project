...
describe("Testy strony http://www.automationpractice.pl/", () =>{           // blok describe opisuje słownie, czym będzie się zajmował test, () => {} - tworzymy funkcję strzałkową i dodajemy blok. => {} to to samo co function()
    it("Wejście na stronę, kliknięcie w zakładkę Contact us", () => {       // it - tworzymy step (krok), a w nim kolejny blok   
        ...
        cy.url().should("include", "http://www.automationpractice.pl/index.php");   //asercja                                                   
    ...
    it("Wpisywanie, usuwanie tekstu w wyszukiwarce i wyszukiwanie", () => {          
        ...
        cy.get('#search_query_top').should("have.value", "przykładowy tekst");      //asercja 
        cy.wait(1000);
    ...
    it("kliknięcie w zakładkę Women", () => {                                                                                                               
        ...
        cy.wait(1000);
        cy.url().should("include", "id_category=3&controller=category");    //asercja 
    })
    
    it("Zaznaczanie/odznaczenie checkboxa w zakładce Women", () => {                                                                
        cy.get('#layered_category_4').check();
        cy.get('#layered_category_4').should("be.checked");                 //asercja                 
        cy.wait(1000);
        cy.get('#layered_category_8').check(); 
        cy.get('#layered_category_8').should("be.checked");                 //asercja                       
        cy.wait(1000);
        cy.get('#layered_category_4').uncheck();                        
        cy.get('#layered_category_8').uncheck();
        cy.get('#layered_category_4').should("not.be.checked");             //asercja 
        cy.get('#layered_category_8').should("not.be.checked");             //asercja                                                
        cy.wait(1000);
    })

    it("Zaznaczanie/odznaczenie grupy checkboxów w zakładce Women", () => {                                                                
        cy.get('#ul_layered_id_attribute_group_1 input').check();
        cy.get('#ul_layered_id_attribute_group_1 input').should("be.checked");          //asercja 
        cy.wait(1000);
        cy.get('#ul_layered_id_attribute_group_1 input').uncheck();
        cy.get('#ul_layered_id_attribute_group_1 input').should("not.be.checked");      //asercja 
        cy.wait(1000);                                                                  
    })

    it("Test związany z wybieraniem select w zakładce Women", () => {                                                                
        cy.get('#selectProductSort').select("In stock");
        cy.get('#selectProductSort').should("have.value", "quantity:desc");             //asercja                                                           
        cy.wait(3000);
        cy.get('#selectProductSort').select("--");
        cy.get('#selectProductSort').should("have.value", "position:asc");              //asercja 
        cy.wait(1000);
        cy.get('#selectProductSort').select("name:asc");
        cy.get('#selectProductSort').should("have.value", "name:asc");                  //asercja        
        cy.wait(3000);  
